...
主にAIX/Linux環境上の銀行システム運用において、影響調査や依頼対応の工数を削減する目的で使用しています。

## **主な機能**

1. **定義取得・環境間コンバート**  
   * ajsprint 結果を取得し、環境間の定義差異（実行エージェントやパスの違いなど）を吸収して変換出力。  
2. **定義回復 (Recover)**  
...
JP1/AJS製造・販売元に対する本ツール開発者の立場は単なる「ユーザー」です。
したがって、本ツールで開発・配布するコードは製造・販売元とは一切関わりがありません。

本ツールで開発・配布するコードは、製造・販売元で公開している定義情報リファレンスに基づき、これを参考にして、定義ファイルのパースやロジックを実装しています。
ただし、同リファレンスの誤読もしくはリファレンスとJP1/AJS間の実装齟齬などにより、実際の動作に差異が存在する可能性があります。

本ツールは個人のポートフォリオとして公開しているものになります。
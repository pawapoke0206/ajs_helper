...
LOG_FILE = LOG_DIR / "print_debug.log"
    ...
    try:
        ...
        with get_ssh_client() as ssh:
            ...
            for i, (fname, rtmp, cmd) in enumerate(cmd_list):
                ...
                # ダウンロードは常にtmpへ（文字コード・改行コード変換を後で行うため）
                lpath = tmp_dir / fname
            ...
            if v_conv_flg.get() == "yes":
                ...
                if not table:
                    _log("[Info] Conversion table is empty. Skipping env conversion.")

            # 文字コード・改行コード変換（+ 環境変換があればそれも適用）
            for path in local_files:
                txt = path.read_text(encoding=srv_enc, errors='ignore')
                for src, dst in table:
                    if src: txt = txt.replace(src, dst)

                out_fname = f"{path.stem}_converted.txt" if table else path.name
                out_path = out_dir / out_fname
                out_path.write_text(txt, encoding=out_c, newline=out_n, errors='ignore')
                _log(f"[Info] Output file saved: {out_path.name} (enc={v_out_c.get()}, nl={v_out_n.get()})")

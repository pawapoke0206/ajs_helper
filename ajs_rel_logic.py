...
# ログファイルパス
LOG_FILE_RUN = LOG_DIR / "rel_run.log"
LOG_FILE_DETAIL = LOG_DIR / "rel_details.json"
...
def pre_parse_graph(txt: str, base: str):
    ...
    # 子孫探索用の「親パス → 直下の子ノードリスト」辞書をグラフに付与する。
    # グラフと辞書のライフサイクルが一致するので、グラフが破棄されれば辞書も消える。
    G.graph['children_map'] = _build_children_map(G)

    return G


def _build_children_map(G):
    """グラフの全ノードから「親パス → 直下の子リスト」の辞書を構築する。
    例: {"/A": ["/A/B", "/A/C"], "/A/B": ["/A/B/D"]}
    """
...
def pre_descendants(G: nx.DiGraph, n):
    """指定ノード配下の子孫ユニットをすべて列挙する（辞書ベースのツリー探索）"""
    children_map = G.graph.get('children_map', {})
    stack = list(children_map.get(n.rstrip('/'), []))
    while stack:
        child = stack.pop()
        yield child
        # さらにその子の配下も辿る
    ...
    try:
        ...
        with get_ssh_client() as ssh:
            ...
            try:
                ...
                try: ssh.exec_command(f"rm -f {tmp_def} {tmp_dep}")
                except: pass
                sftp.close()

...
# ログファイルパス
LOG_FILE = LOG_DIR / "define_debug.log"
...
def define_convert_newlines(local_path, target_enc):
    ...
    ファイルの文字コードはUTF-8/cp932を自動判定する。
    アップロード時はtarget_enc（サーバ設定の文字コード）でエンコードされる。
    ...
    _log(f"[Info] File read: detected={detected_enc}, will upload as={target_enc}")
    return content
